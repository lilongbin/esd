...
=================================================
    ...
    |_______|       |_______|       |_______|
        ^               ^               ^
        |               |               |
   二级指针变量  一级指针变量p_num  普通变量num
=================================================
地址b中存放的数据是地址a(普通变量的地址)即一级指针变量的值;
	p_num == &num;
...
指针变量也是分类型的,一种类型的指针变量为一种类型的普通变量服务;
	...
	既没有记录有效地址也没有记录空地址的指针叫野指针;
	要避免程序中出现野指针;也叫悬空指针;
约定,没有记录普通存储位置地址的指针变量中必须记录NULL(空地址,数值为0);
	避免野指针的办法是在它刚出现的时候立刻把它的内容设置成空地址;
	所有的指针变量都应当初始化;
...
野指针不是和没有赋值的变量一样吗?感觉不到什么威胁啊;
	...
	定义指针时如果不赋初值,系统会随机赋个地址给他,如果这个地址在程序的代码区,在地址变量被引用时会操作运行的程序代码,会出错;
把NULL指针用%d打印出来就会段错误;
