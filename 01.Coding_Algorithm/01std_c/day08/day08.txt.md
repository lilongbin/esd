...
每一个存储位置有一个自己的编号,最前边的存储位置的编号是0,向后依次递增,最后一个存储位置的编号是个数-1;
这个编号叫下标;
绝不可以使用超过范围的下标;如果使用了越界的存储位置结果是未知的,不能保证安全性;
我们可以使用名称[下标]就可以表示这组存储位置中的某一个;
...
如果初始化的时候为每个存储位置提供了数据则可以省略存储位置的个数,例如
int arr[] = {1, 2, 3};
...
数组中下标为n的存储位置的地址可以使用数组名称加n的方式表示
...
int main() {
	...
	printf("&arr[1]是%p, arr+1是%p\n", &arr[1], arr + 1);
	/* &arr[1]是0xbfb6ee88, arr+1是0xbfb6ee88 */

	printf("&arr[1]是%x, arr + 1是%x\n", &arr[1], arr + 1);
	/* &arr[1]是bfb6ee88, arr+1是bfb6ee88 */

	printf("arr是%x, &arr是%x, &arr[0]是%x\n", arr, &arr, &arr[0]);
	/* arr是bfb6ee84, &arr是bfb6ee84, &arr[0]是bfb6ee84 */
...
变长数组不可以初始化;
数组一旦存在,其存储位置的个数就不可改变了,数组的大小是不可调的;
...
time(0);或time(NULL);将只以返回值的形式获得当前时间的秒数;
...
只使用一个下标就可以表示某个存储位置的数组叫一维数组
使用两个下标表示某个存储位置的数组叫二维数组
二维数组也是对一串连续存储位置的表示方式,对这些存储位置是不会有什么变化的;
...
二维数组即可以当成一个一维数组来初始化,也可以当成多个一维数组进行初始化
如果可以根据初始化数据的个数确定组的个数则可以省略组的个数

二维数组只使用一个下标可以表示某一组中所有存储位置;
如arr[0]代表的是一个地址;
...
#include <stdio.h>
int main() {
	//int arr[2][3] = {1, 2, 3, 4, 5, 6}; //当成1个一维数组初始化
	//int arr[2][3] = {1, 2, 3, 4}; 
	//int arr[2][3] = {{1, 2}, {3, 4}}; //当成2个一维数组
	//int arr[][3] = {1, 2, 3, 4}; //可以在适当情况下省略大小数字
	int arr[][3] = { {1, 2}, {3, 4} };
	int row = 0, col = 0;
