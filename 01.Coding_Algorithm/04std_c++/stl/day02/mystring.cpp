...
std::string lstrip(const std::string &rstr, const std::string &delim="")
{
    size_t start = 0;
    size_t slen = rstr.length();
    std::string sps = delim;
    const size_t spl = sps.length();

    if (0 == spl) {
        sps = "\r\n \t\v\f";
        // skip head char startswith any of sp
        while ((slen >= 1)
                && (std::string::npos != sps.find(rstr.at(start)))) {
            start += 1;
            slen -= 1;
        }
    } else {
        // skip head char startswith sp
        while ((slen >= spl)
                && (0 == rstr.compare(start, spl, sps))) {
            start += spl;
            slen -= spl;
        }
    }
    return rstr.substr(start, slen);
}

std::string rstrip(const std::string &rstr, const std::string &delim="")
{
    size_t start = 0;
    size_t slen = rstr.length();
    std::string sps = delim;
    ...
    if (0 == spl) {
        sps = "\r\n \t\v\f";
        // remove tail char endswith any of sp
        while ((slen >= 1)
                && (std::string::npos != sps.find(rstr.at(start+slen-1)))) {
            slen -= 1;
        }
    } else {
        // remove tail char endswith sp
        while ((slen >= spl)
                && (0 == rstr.compare((start+slen-spl), spl, sps))) {
            slen -= spl;
        }
    }
    return rstr.substr(start, slen);
...
std::string strip(const std::string &rstr, const std::string &delim="")
{
#if 1
    size_t start = 0;
    size_t slen = rstr.length();
    ...
    return rstr.substr(start, slen);
#else
    return rstrip(lstrip(rstr, delim), delim);
#endif
}
...
string mask(const string &src, int begin, int masklen)
{
    string ret = src;
    string mask = "";
    ...
    if (masklen >= 0)
    {
        // std::min(masklen, (length-pos))
        replen = ((length - pos) > masklen) ? masklen : (length - pos);
    }
    else
    {
        // mask from tail
        // std::min((-masklen), (pos+1));
...
int main() {
    ...
    print(rstrip(str));
    print(strip(str));
    str = "123456789001234567890";
    print(str);
    print(lstrip(str, "1234567890"));
    print(rstrip(str, "1234567890"));
    print(strip(str, "1234567890"));
    ...
    // startsWith
    print(startsWith(test, "01234")); //1
    // endsWith
    print(endsWith(test, "7890")); //1
    cout << split_line << endl;
    return 0;
}
